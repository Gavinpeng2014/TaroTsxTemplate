...
Component({
  ...
  methods: {
    ...
    setContent: function setContent(html, append) {
      ...
      var nodes,
          parser = new Parser(html, this.data);
      // 缓存读取
      if (this.data.useCache) {
        var hashVal = hash(html);
        if (cache[hashVal]) nodes = cache[hashVal];else cache[hashVal] = nodes = parser.parse();
      } else nodes = parser.parse();
      this.triggerEvent('parse', nodes);
      var data = {};
      if (append) for (var i = this.data.nodes.length, j = nodes.length; j--;) {
      ...
      // 设置标题
      if (nodes.title && this.data.autosetTitle) wx.setNavigationBarTitle({
        title: nodes.title
      });
      this.imgList.length = 0;
      this.videoContexts = [];
      var ns = this.selectAllComponents('.top,.top>>>._node');
      for (var _i = 0, n; n = ns[_i++];) {
        n.top = this;
        for (var _j = 0, item; item = n.data.nodes[_j++];) {
          if (item.c) continue;
          // 获取图片列表
